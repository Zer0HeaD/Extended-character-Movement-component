...
bool UExtendedCharacterMovementComp::DoJump(bool bReplayingMoves)
{
...
void UExtendedCharacterMovementComp::UpdateCharacterStateBeforeMovement(float DeltaSeconds)
{
	...
	if (Safe_bTransitionFinished)
	{
		...
		if (IsValid(TransitionQueuedMontage) && IsValid(FPSTransitionQueuedMontage))
		{
			SetMovementMode(MOVE_Flying);
			CharacterOwner->PlayAnimMontage(TransitionQueuedMontage, TransitionQueuedMontageSpeed);
			ExtendedCharacterOwner->FPSPlayTransitionQueuedMontage();
			TransitionQueuedMontageSpeed = 0.f;
			TransitionQueuedMontage = nullptr;
			FPSTransitionQueuedMontage = nullptr;
		}
		else
		{
			SetMovementMode(MOVE_Walking);
...
{
	return IsCustomMovementMode(CMOVE_Prone) || IsMovementMode(MOVE_Walking) && IsCrouching();
}
...
void UExtendedCharacterMovementComp::PerformDash()
{
...
SLOG(FString::Printf(TEXT("Duration: %f"), TransitionRMS->Duration))
	...
	if (bTallMantle)
	{
		...
		FPSTransitionQueuedMontage = FPSTallMantleMontage;

		ExtendedCharacterOwner->FPSPlayTransitionTallMontage();
	...
	else
	{
		...
		FPSTransitionQueuedMontage = FPSShortMantleMontage;

		ExtendedCharacterOwner->FPSPlayTransitionShortMontage();
		CharacterOwner->PlayAnimMontage(TransitionShortMantleMontage, 1);  // 1 / TransitionRMS->Duration
